...
import { appointmentStatus, parseSlotDateTime } from '@/lib/appointment';
...
export class AppointmentContextService {
    public static async getContext(userId: string) {
        try {
            ...
            // Shares the parser with the rest of the app, so the AI describes an
            // appointment with the same status the patient sees on their dashboard.
            const parsedAppointments = appointments.map(app => ({
                ...app,
                parsedDate: parseSlotDateTime(app.slotDate, app.slotTime) ?? new Date(0),
            }));
